...
/**
 * Node representing an entire document. It inherits {@linkplain Node} and stores children nodes for this document.
 * @author Andi Škrgat
 * @version 1.0
 */
public class DocumentNode extends Node{
	...
	/**
	 * Method that toString uses. It recursively passes children and appends node's text to string builder.
	...
	private StringBuilder intern(Node child, StringBuilder sb) {
		...
		try {
			...
			//System.out.println(ex.getLocalizedMessage()); // null pointer happens because intern collection is allocated only when element is first time
			//added to collection
		}
