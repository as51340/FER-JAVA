...
public class ArrayIndexedCollection<E> implements List<E>{
	...
	/**
	 * defaultCapacity of our collection
	 */
	...
	/**
	 * Array of objects, where elements are internally stored
	 */
	...
	public ElementsGetter<E> createElementsGetter() {
		return new ArrayIndexedCollectionElementsGetter<E>(this);
	}
	 ...
	 * @author Andi Škrgat
	 */
	private static class ArrayIndexedCollectionElementsGetter<E> implements ElementsGetter<E> {
		
		/**
		 * Index saves current position in collection
		 */
		private int index = 0;
		
		/**
		 * Instance of collection over which we are iterating.
		 */
		private ArrayIndexedCollection<E> col;
		
		/**
		 * Variable with which we can see if outside modification has been made.
		 */
		 ...
		 */
		private ArrayIndexedCollectionElementsGetter(ArrayIndexedCollection<E> col) {
			this.col = col;
